...
export function InformacionGeneral() {
  ...
  const generalGCDetalleInversion: { Id: string, Descripcion: string } =
    useLargoPlazoStore(
      (state) => state.generalGastosCostos.detalleInversion
    );
  ...
  const [periodoDeAdministracion, setPeriodoAdministracion] = useState(generalGCperiodoAdministracion)
  const [gastosAdicionalesGC, setGastosAdicionalesGC] = useState(generalGCGastosAdicionales)
  const [claveInscripcionFinanciamiento, setClaveInscripcionFinanciamiento] = useState(generalGCClaveInscripcionFinanciamiento)
  const [descripcionGC, setDescripcionGC] = useState(generalGCDescripcion)
  const [montoGC, setMontoGC] = useState(generalGCMonto)
  const [periodoFinanciamientoGC, setPeriodoFinanciamiento] = useState(generalGCPeriodoFinanciamiento)
  const [saldoVIgenteGC] = useState(generalGCSaldoVigente)
  const [montoGastosAdicionalesGC, setMontoGastosAdicionalesGC] = useState(generalGCMontoGastosAdicionales)
    ...
    >
      ...
      <Grid item display={"flex"} width={"100%"} justifyContent={"center"}>
        <Grid width="87.5%" display={"flex"} justifyContent={"space-between"}>
          <Grid item lg={4.2} >
            <TextField disabled sx={{ width: "80%" }} label="Periodo de administración" variant="standard" />
          </Grid>
          <Grid item lg={3.4}>
            <TextField disabled sx={{ width: "100%" }} label="Periodo de financiamiento" variant="standard" />
          </Grid>
        </Grid>
      </Grid>
      ...
      <Grid item display={"flex"} justifyContent={"space-evenly"}  >
        ...
        <Grid item lg={3}>
          <InputLabel sx={queries.medium_text}>Detalle de la Inversión</InputLabel>
          <Autocomplete
            clearText="Borrar"
            noOptionsText="Sin opciones"
            closeText="Cerrar"
      ...
      <Grid item display={"flex"} justifyContent={"space-evenly"}  >
        ...
        <Grid item lg={3}>
          ...
          <TextField
            fullWidth
            placeholder="0"
            value={generalGCGastosAdicionales <= 0 ? "" : generalGCGastosAdicionales.toString()}
            onChange={(v) => {
              if (validator.isNumeric(v.target.value)) {
                changeGeneralGastosCostos({
                  destino: generalGCDestino,
      ...
      <Grid item display={"flex"} justifyContent={"space-evenly"}  >
        ...
        <Grid item lg={3}>
          ...
          <TextField
            fullWidth
            placeholder="0"
            value={generalGCMontoGastosAdicionales <= 0 ? "" : generalGCMontoGastosAdicionales.toString()}
            onChange={(v) => {
              if (validator.isNumeric(v.target.value)) {
                changeGeneralGastosCostos({
                  destino: generalGCDestino,
