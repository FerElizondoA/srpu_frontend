...
module.exports = {
    createRespecto: (req, res) => {
        ...
        if (
          ...
          db.query(
            `CALL sp_AgregarRespecto('${IdUsuario}', '${Descripcion}' )`,
            (err, result) => {
              if (err) {
      ...
      //LISTADO COMPLETO
      getRespecto: (req, res) => {
        db.query(`CALL sp_ListadoRespecto()`, (err, result) => {
          if (err) {
            return res.status(500).send({
              error: "Error",
            });
      ...
      modifyRespecto: (req, res) => {
        ...
        if (IdUsuarioModificador == null || /^[\s]*$/.test(IdUsuarioModificador)) {
          ...
          db.query(
            `CALL sp_ModificaRespecto('${IdDescripcion}','${Descripcion}','${IdUsuarioModificador}')`,
            (err, result) => {
              if (err) {
                return res.status(500).send({
                  error: "Error",
      ...
      deleteRespecto: (req, res) => {
        ...
        db.query(
          `CALL sp_BajaLogicaRespecto('${IdDescripcion}', '${IdUsuarioModificador}')`,
          (err, result) => {
            if (err) {
              return res.status(500).send({
                error: "Error",
