...
module.exports = {
  ...
  createTipoDeFuente: (req, res) => {
    ...
    if (
      ...
      db.query(
        `CALL sp_AgregarTipoDeFuente('${IdUsuario}', '${Descripcion}' )`,
        (err, result) => {
          if (err) {
  ...
  getTiposDeFuente: (req, res) => {
    db.query(`CALL sp_ListadoTiposDeFuente()`, (err, result) => {
      if (err) {
        return res.status(500).send({
          error: "Error",
        });
  ...
  getDetailTipoDeFuente: (req, res) => {
    ...
    db.query(
      `CALL sp_DetalleTipoDeFuente('${IdDescripcion}')`,
      (err, result) => {
        if (err) {
          return res.status(500).send({
            error: "Error",
  ...
  modifyTipoDeFuente: (req, res) => {
    ...
    if (Descripcion == null || /^[\s]*$/.test(Descripcion)) {
      return res.status(409).send({
        error: "Ingrese Nuevo TipoDeFuente",
      });
    }
    ...
    if (IdUsuarioModificador == null || /^[\s]*$/.test(IdUsuarioModificador)) {
      ...
      db.query(
        `CALL sp_ModificaTipoDeFuente('${IdDescripcion}','${Descripcion}','${IdUsuarioModificador}')`,
        (err, result) => {
          if (err) {
            return res.status(500).send({
              error: "Error",
  ...
  deleteTipoDeFuente: (req, res) => {
    ...
    db.query(
      `CALL sp_BajaLogicaTipoDeFuente('${IdDescripcion}', '${IdUsuarioModificador}')`,
      (err, result) => {
        if (err) {
          return res.status(500).send({
            error: "Error",
