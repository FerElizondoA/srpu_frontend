...
module.exports = {
  ...
  createOrdenFideicomisario: (req, res) => {
    ...
    if (
      ...
      db.query(
        `CALL sp_AgregarOrdenFideicomisario('${IdUsuario}', '${Descripcion}' )`,
        (err, result) => {
          if (err) {
  ...
  getOrdenesFideicomisario: (req, res) => {
    db.query(`CALL sp_ListadoOrdenesFideicomisario()`, (err, result) => {
      if (err) {
        return res.status(500).send({
          error: "Error",
        });
  ...
  getDetailOrdenFideicomisario: (req, res) => {
    ...
    db.query(
      `CALL sp_DetalleOrdenFideicomisario('${IdDescripcion}')`,
      (err, result) => {
        if (err) {
          return res.status(500).send({
            error: "Error",
  ...
  modifyOrdenFideicomisario: (req, res) => {
    ...
    if (Descripcion == null || /^[\s]*$/.test(Descripcion)) {
      return res.status(409).send({
        error: "Ingrese Nuevo OrdenFideicomisario",
      });
    }
    ...
    if (IdUsuarioModificador == null || /^[\s]*$/.test(IdUsuarioModificador)) {
      ...
      db.query(
        `CALL sp_ModificaOrdenFideicomisario('${IdDescripcion}','${Descripcion}','${IdUsuarioModificador}')`,
        (err, result) => {
          if (err) {
            return res.status(500).send({
              error: "Error",
  ...
  deleteOrdenFideicomisario: (req, res) => {
    ...
    db.query(
      `CALL sp_BajaLogicaOrdenFideicomisario('${IdDescripcion}', '${IdUsuarioModificador}')`,
      (err, result) => {
        if (err) {
          return res.status(500).send({
            error: "Error",
