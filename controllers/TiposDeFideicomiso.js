...
module.exports = {
  ...
  createTipoDeFideicomiso: (req, res) => {
    ...
    if (
      ...
      db.query(
        `CALL sp_AgregarTipoDeFideicomiso('${IdUsuario}', '${Descripcion}' )`,
        (err, result) => {
          if (err) {
  ...
  getTiposDeFideicomiso: (req, res) => {
    db.query(`CALL sp_ListadoTiposDeFideicomiso()`, (err, result) => {
      if (err) {
        return res.status(500).send({
          error: "Error",
        });
  ...
  getDetailTipoDeFideicomiso: (req, res) => {
    ...
    db.query(
      `CALL sp_DetalleTipoDeFideicomiso('${IdDescripcion}')`,
      (err, result) => {
        if (err) {
          return res.status(500).send({
            error: "Error",
  ...
  modifyTipoDeFideicomiso: (req, res) => {
    ...
    if (IdUsuarioModificador == null || /^[\s]*$/.test(IdUsuarioModificador)) {
      ...
      db.query(
        `CALL sp_ModificaTipoDeFideicomiso('${IdDescripcion}','${Descripcion}','${IdUsuarioModificador}')`,
        (err, result) => {
          if (err) {
            return res.status(500).send({
              error: "Error",
  ...
  deleteTipoDeFideicomiso: (req, res) => {
    ...
    db.query(
      `CALL sp_BajaLogicaTipoDeFideicomiso('${IdDescripcion}', '${IdUsuarioModificador}')`,
      (err, result) => {
        if (err) {
          return res.status(500).send({
            error: "Error",
