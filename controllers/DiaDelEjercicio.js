...
module.exports = {
  ...
  createDiaDelEjercicio: (req, res) => {
    ...
    if ((IdUsuario == null || /^[\s]*$/.test(IdUsuario)) && IdUsuario.length() <= 36) {
      ...
      db.query(
        `CALL sp_AgregarDiaDelEjercicio('${IdUsuario}', '${Descripcion}' )`,
        (err, result) => {
          if (err) {
  ...
  getDiasDelEjercicio: (req, res) => {
    db.query(`CALL sp_ListadoDiasDelEjercicio()`, (err, result) => {
      if (err) {
        return res.status(500).send({
          error: "Error",
        });
  ...
  getDetailDiaDelEjercicio: (req, res) => {
    ...
    db.query(
      `CALL sp_DetalleDiaDelEjercicio('${IdDescripcion}')`,
      (err, result) => {
        if (err) {
          return res.status(500).send({
            error: "Error",
  ...
  modifyDiaDelEjercicio: (req, res) => {
    ...
    if (IdUsuarioModificador == null ||/^[\s]*$/.test(IdUsuarioModificador)) {
      ...
      db.query(
        `CALL sp_ModificaDiaDelEjercicio('${IdDescripcion}','${Descripcion}','${IdUsuarioModificador}')`,
        (err, result) => {
          if (err) {
            return res.status(500).send({
              error: "Error",
  ...
  deleteDiaDelEjercicio: (req, res) => {
    ...
    db.query(
      `CALL sp_BajaLogicaDiaDelEjercicio('${IdDescripcion}', '${IdUsuarioModificador}')`,
      (err, result) => {
        if (err) {
          return res.status(500).send({
            error: "Error",
