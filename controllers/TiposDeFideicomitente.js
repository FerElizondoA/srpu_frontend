...
module.exports = {
  ...
  createTipoDeFideicomitente: (req, res) => {
    ...
    if (
      ...
      db.query(
        `CALL sp_AgregarTipoDeFideicomitente('${IdUsuario}', '${Descripcion}' )`,
        (err, result) => {
          if (err) {
  ...
  getTiposDeFideicomitente: (req, res) => {
    db.query(`CALL sp_ListadoTiposDeFideicomitente()`, (err, result) => {
      if (err) {
        return res.status(500).send({
          error: "Error",
        });
  ...
  getDetailTipoDeFideicomitente: (req, res) => {
    ...
    db.query(
      `CALL sp_DetalleTipoDeFideicomitente('${IdDescripcion}')`,
      (err, result) => {
        if (err) {
          return res.status(500).send({
            error: "Error",
  ...
  modifyTipoDeFideicomitente: (req, res) => {
    ...
    if (Descripcion == null || /^[\s]*$/.test(Descripcion)) {
      return res.status(409).send({
        error: "Ingrese Nuevo TipoDeFideicomitente",
      });
    }
    ...
    if (IdUsuarioModificador == null || /^[\s]*$/.test(IdUsuarioModificador)) {
      ...
      db.query(
        `CALL sp_ModificaTipoDeFideicomitente('${IdDescripcion}','${Descripcion}','${IdUsuarioModificador}')`,
        (err, result) => {
          if (err) {
            return res.status(500).send({
              error: "Error",
  ...
  deleteTipoDeFideicomitente: (req, res) => {
    ...
    db.query(
      `CALL sp_BajaLogicaTipoDeFideicomitente('${IdDescripcion}', '${IdUsuarioModificador}')`,
      (err, result) => {
        if (err) {
          return res.status(500).send({
            error: "Error",
